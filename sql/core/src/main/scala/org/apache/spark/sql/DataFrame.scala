...
class DataFrame private[sql](
   ...
   */
  def this(sqlContext: SQLContext, logicalPlan: LogicalPlan, source: String = "Spark,,,") = {
    this(sqlContext, {
  ...
  /**
    * Give the datasource vale,Mingda
    */
  def source(args: String*): DataFrame = {
    ...
    new DataFrame(this.sqlContext, this.logicalPlan, s)
  }

  /**
    * Output datasource,Mingda
    */
  def showSource(): String = {
    this.source
  }
  ...
  def join(right: DataFrame): DataFrame = {
    val s = right.showSource() // this dataframe's source end with ,
    if(s!= null){
      withPlan {
        Join(logicalPlan, right.logicalPlan, joinType = Inner, None)
      }.source(this.source + right.showSource().dropRight(2))
    }
    withPlan {
      Join(logicalPlan, right.logicalPlan, joinType = Inner, None)
    }
  ...
  def join(right: DataFrame, joinExprs: Column, joinType: String): DataFrame = {
    ...
    if (!sqlContext.conf.dataFrameSelfJoinAutoResolveAmbiguity) {
      return withPlan(plan).source(this.source + right.showSource().dropRight(2))
    }
    ...
    }.source(this.source + right.showSource().dropRight(2)) // Right side has , which will cause duplicate ,
  }
  ...
  //Added Mingda
  // logicalRelation not in show, in one query
  def operateLogicalRelation (child:LogicalPlan, dataverse: String, dataset: String): String ={
    child match{
      case LogicalRelation(relation, expectedOutputAttributes) => dataset
      case _ => "(%s)m%d".format(logicalplanToSQLPP(child, dataverse, dataset), scala.util.Random.nextInt(10000)) // this.source(source.split(","){})

    }
  }
  // Convert the comparision of a > b to String or if b is integer to String
  // Other just empty
  def expressionToSQLPP (ex: Expression): String = {
    ex match{
      case AttributeReference(name, dataType, nullable, metadata) => name
      case Literal(value , dataType) => value.toString
      case Cast(child, dataType) => expressionToSQLPP(child)
      case _ => " "

    }
  }
  def filterToSQLPP (condition: Expression, child: LogicalPlan, dataverse:String, dataset:String ): String = {
    condition match {
      case LessThan(left, right) => ("SELECT * FROM %s " +
        "where %s > %s ").format(operateLogicalRelation(child, dataverse, dataset),
        expressionToSQLPP(left), expressionToSQLPP(right))
      case GreaterThan(left, right) => ("SELECT * FROM %s " +
        "where %s > %s ").format(operateLogicalRelation(child, dataverse, dataset),
        expressionToSQLPP(left), expressionToSQLPP(right))
      case EqualTo(left, right) => ("SELECT * FROM %s " +
        "where %s = %s ").format(operateLogicalRelation(child, dataverse, dataset),
        expressionToSQLPP(left), expressionToSQLPP(right))
      case _ => " "
  ...
  def limitToSQLPP (limitExpr :Expression) : Any = {
    limitExpr match {
      case Literal(value, dataType) => value
      case _ => " "
    }
  }
  def attributeRefer(a : Expression): String = a match{
    case AttributeReference(name, dataType, nullable, metadata) => name
    case _ => " "
  }


  def joinConditionToSQLPP(condition: Option[Expression]): String ={
    if(condition.isDefined){
      condition.get match {
        case EqualTo(left, right) => "(%s = %s)".format( attributeRefer(left), attributeRefer(right))
        case _ => " "
      }
    }else{
      " "
  ...
  def joinToSQLPP (left: LogicalPlan, right: LogicalPlan, joinType: JoinType, condition: Option[Expression], dataverse: String, dataset: String): String = joinType match{
    case Inner => "SELECT * FROM %s JOIN %s ON %s".format(operateLogicalRelation(left, dataverse, dataset),
      operateLogicalRelation(right, this.source.split(","){4}, this.source.split(","){5}),
      joinConditionToSQLPP(condition))
    case _ => ""
  }
  def projectListToSQLPP (projectList: Seq[NamedExpression]) :String ={
    var s = "  "
    for( a <- projectList){
      a match{
        case AttributeReference(name, dataType, nullable, metadata) => s = s+ name + ", "
        case _ => ""
      }
    }
    s.dropRight(2)
  }
  def logicalplanToSQLPP (logicalPlan: LogicalPlan, dataverse: String, dataset: String): String = logicalPlan match {
    case Project(projectList, child) => "SELECT %s FROM (%s)m%d".format(projectListToSQLPP(projectList), logicalplanToSQLPP(child, dataverse, dataset), scala.util.Random.nextInt(10000))
    case LogicalRelation(relation, exepectedOutputAttributes) => "SELECT * FROM %s".format(dataset)
    case Limit(limitExpr, child) => "%s LIMIT %s".format(logicalplanToSQLPP(child, dataverse, dataset), limitToSQLPP(limitExpr).toString)
    case Filter(condition, child) => filterToSQLPP(condition, child, dataverse, dataset)
    case Join(left, right, joinType, condition) => joinToSQLPP(left, right, joinType, condition, dataverse, dataset)
    case _ => ""
  }
  def RunSQLPP (spp : String) :DataFrame = {
    this.sqlContext.sqlpp(spp)
  ...
  private def collect(needCallback: Boolean): Array[Row] = {
    ...
    */
    ...
    if (this.source.split(","){0} contains  "AsterixDB") {
            val dataverse = this.source.split(","){1} // Only the first three of Source means the dataframe's variables.
          val dataset = this.source.split(","){2} // The second three are the table's information he join with.
          var sqlPlusPlus = logicalplanToSQLPP(queryExecution.logical, dataverse, dataset)
          val limit = 20
          sqlPlusPlus = "USE %s;  %s;".format(dataverse, sqlPlusPlus)
          // scalastyle:off
          println(" sql++ query")
          println(sqlPlusPlus)
          // scalastyle:on
          RunSQLPP(sqlPlusPlus).collect()
    }
  ...
  def count(): Long = {
    if(this.source.split(","){0} contains("AsterixDB")){
      ...
      val dataset = this.source.split(","){2} // The second three are the table's information he join with.
      var sqlPlusPlus = logicalplanToSQLPP(queryExecution.logical, dataverse, dataset)
      sqlPlusPlus = "USE %s; SELECT COUNT(*) FROM (%s)m%d;".format(dataverse, sqlPlusPlus,scala.util.Random.nextInt(10000))
      // scalastyle:off
      println(" sql++ query")
      println(sqlPlusPlus)
      // scalastyle:on
      RunSQLPP(sqlPlusPlus).collect(needCallback = false).head.getLong(0)
    }
    else{
  ...
  @inline private def withPlan(logicalPlan: => LogicalPlan): DataFrame = {
    new DataFrame(sqlContext, logicalPlan, this.source)
    // Mingda's adding the source in to convert dataframe
  }
